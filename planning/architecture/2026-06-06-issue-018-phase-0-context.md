...
> **RESOLVED 2026-08-16.** The gate this note names (ISSUE-031) drained; ISSUE-018
> was implemented per §6's resume plan: intelligence_reader posture
> version-controlled (`20260816_issue018_intelligence_reader_grant_provision.sql`),
> dedicated fail-closed pool (`db.ts getIntelligencePool`/`withIntelligenceOrgContext`),
> the risk-map job moved wholly onto the channel (D-D's "reads-that-need-writes"
> resolved by granting DML on the job's two identity-free derived outputs), runtime
> boundary tests added. D-A landed earlier the same day as ISSUE-028/Q-F. D-B (PG14
> view-owner bridge) re-decided: accepted — post-clip granted views are identity-free
> and no view reaches a sidecar; PG15 stays ISSUE-029. This file landed on main from
> the paused branch as the historical record. Changelog:
> `docs/changelog/security/2026-08-16-issue018-intelligence-channel.md`.
