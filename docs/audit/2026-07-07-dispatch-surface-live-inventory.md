...
> **⚠️ POINT-IN-TIME SNAPSHOT (landed on main 2026-09-17, DOCS-6).** This is the
> 2026-07-07 discovery artifact copied verbatim off the unmerged
> `chore/dispatch-discovery-inventory` branch — every verdict below describes
> main @ `779bdbb`, NOT current state. Known supersessions since: the Control
> Center **has** relocated to Dispatch (`/ops/control-center`, SEAM-B, merged
> 2026-07-10), and the "reads a clipped adapter" findings were resolved by the
> SEAM-C repoints + ISSUE-037 adapter-table drops. Treat everything below as
> history; verify against current code before acting on any claim.
