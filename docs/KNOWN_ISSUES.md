...
**Status:** Root fixed in the runner 2026-06-23 (CI workaround still in place) — see Update below  
**Update (2026-06-23):** the ROOT — a fresh init leaving `fieldpro` a superuser, which silently disables FORCE-RLS — is now **permanently fixed in version control**. Migration `backend/migrations/20260624_role_provisioning_codify.sql` codifies `ALTER ROLE fieldpro NOSUPERUSER NOBYPASSRLS` (idempotent) plus a least-privilege `fieldpro_admin` provisioner, so a fresh environment reproduces the non-superuser app role from code instead of relying on each operator to hand-downgrade it. The CI `fieldpro_test` workaround (below) remains valid and complementary. The empty→migrate gate proof of `20260624` is pending a provisioner credential (see `docs/audit/2026-06-23-role-provisioning-fix.md` Phase 2/3). The original CI/app-connection-role decision still routes through ISSUE-018.  
